...
for iteration in range(10):  # Run for 5 iterations
    # fit the gp model
    #kernel1 = kernel.ARDKernel(1, initial_length_scale=2.1)
    #model = CIGP_withMean(1, 1, kernel=kernel1, noise_variance=1.0)
    ...
    batch_points = pi.find_next_batch(bounds, batch_size=1, n_samples=500, f_best=train_x[np.argmax(train_y)])
